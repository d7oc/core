...
Feature: copy file
  ...
  Scenario Outline: Copy file into a not-existing folder
    Given using <dav_version> DAV path
  ...
  Scenario Outline: user tries to copy a file that doesn't exist into a folder
    Given using <dav_version> DAV path
  ...
  Scenario Outline: Copying a file between shares received from different users
    ...
    When user "Alice" copies file "/textfile0.txt" to "/Shares/testshare0/textfile0.txt" using the WebDAV API
    And user "Alice" copies file "/textfile1.txt" to "/Shares/testshare1/textfile1.txt" using the WebDAV API
    Then the HTTP status code of responses on all endpoints should be "201"
    And as "Brian" file "testshare0/textfile0.txt" should exist
    And as "Carol" file "testshare1/textfile1.txt" should exist
    And the content of file "/testshare0/textfile0.txt" for user "Brian" should be "ownCloud test text file 0"
    And the content of file "/testshare1/textfile1.txt" for user "Carol" should be "ownCloud test text file 1"
    Examples:
      | dav_version |
      | old         |
      | new         |
  ...
  @files_sharing-app-required
  Scenario Outline: Copying a file between shares to different users
    Given using <dav_version> DAV path
    And user "Brian" has been created with default attributes and without skeleton files
    And user "Carol" has been created with default attributes and without skeleton files
  ...
  Scenario: Copy a file into storage
    Given user "Alice" has created folder "/local_storage/foo1"
  ...
  Scenario Outline: Copy a folder into an other one
    Given using <dav_version> DAV path
    And user "Alice" has created folder "/testshare"
